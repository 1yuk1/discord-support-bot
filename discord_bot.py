...
MESSAGE_DEBOUNCE_SECONDS = config.MESSAGE_DEBOUNCE_SECONDS
...
Донат и оплата:
- Купить донат можно только на https://sinussmp.com или через Boosty https://boosty.to/ingrog.
- easyDonate — это платежная система внутри сайта https://sinussmp.com, а не отдельный магазин доната.
...
- Для покупки через сайт https://sinussmp.com: оплата проходит через easyDonate, донат должен выдаться автоматически. Если донат не пришел, сначала попроси перезайти на сервер/режим, указанный при оплате (Лайт или Практайз).
...
- Если в КОНТЕКСТЕ нет цены конкретного товара — направь игрока на https://sinussmp.com, не выдумывай цифры.
...
def create_channel_state():
    return {
        ...
        # Склейка спама: части сообщений + задача debounce.
        "pending_parts": [],
        "debounce_task": None,
        "ai_busy": False,
    }
...
def _cancel_debounce_task(channel_data):
    task = channel_data.get("debounce_task")
    if task is not None and not task.done():
        task.cancel()
    channel_data["debounce_task"] = None


def _merge_pending_parts(parts):
    texts = []
    images = []
    author_is_bot = False
    for part in parts:
        ...
        for url in part.get("image_urls") or []:
            if url not in images:
                images.append(url)
        if part.get("author_is_bot"):
            author_is_bot = True
    merged_texts = []
    for text in texts:
        if not merged_texts or _normalize_for_dedup(merged_texts[-1]) != _normalize_for_dedup(text):
            merged_texts.append(text)
    return "\n".join(merged_texts).strip(), images, author_is_bot


async def _run_ai_reply(channel, channel_data, message_text, image_urls, author_is_bot):
    """Один AI-ответ на (уже склеенный) текст."""
    channel_id = getattr(channel, "id", "unknown")

    if channel_data.get("human_mode"):
        return
    if channel_data.get("ai_busy"):
        logger.info("AI уже отвечает в канале, пропуск | channel_id=%s", channel_id)
        return
    ...
    if cooldown_remaining > 0:
        await safe_send(channel, f"⏳ Подождите {cooldown_remaining} секунд перед следующим вопросом.")
        return

    if not check_rate_limit():
        await safe_send(channel, "⏳ Слишком много сообщений. Подожди минуту.")
        return

    logger.info(
        "Начинаем генерацию ответа AI | channel_id=%s | concurrency_limit=%s | text_preview=%s",
        channel_id,
        max(AI_MAX_CONCURRENT_REQUESTS, 1),
        message_text[:200].replace("\n", " "),
    )

    channel_data["ai_busy"] = True
    try:
...
async def _debounce_flush(channel, channel_data):
    """После паузы склеивает pending-сообщения и отвечает один раз."""
    try:
        await asyncio.sleep(max(MESSAGE_DEBOUNCE_SECONDS, 0.1))
    except asyncio.CancelledError:
        return

    parts = channel_data.get("pending_parts") or []
    channel_data["pending_parts"] = []
    channel_data["debounce_task"] = None

    if not parts or channel_data.get("human_mode"):
        return

    message_text, image_urls, author_is_bot = _merge_pending_parts(parts)
    ...
    logger.info(
        "Debounce flush | channel_id=%s | parts=%s | text_preview=%s",
        getattr(channel, "id", "unknown"),
        len(parts),
        message_text[:200].replace("\n", " "),
    )
    await _run_ai_reply(channel, channel_data, message_text, image_urls, author_is_bot)


def _queue_message_for_ai(channel, channel_data, message_text, image_urls, author_is_bot):
    """Добавляет сообщение в буфер и (пере)запускает debounce."""
    if MESSAGE_DEBOUNCE_SECONDS <= 0:
        return False

    channel_data.setdefault("pending_parts", []).append({
        "text": message_text,
        "image_urls": list(image_urls or []),
        "author_is_bot": bool(author_is_bot),
    })

    _cancel_debounce_task(channel_data)
    channel_data["debounce_task"] = asyncio.create_task(
        _debounce_flush(channel, channel_data)
    )
    logger.info(
        "Сообщение в debounce-буфере | channel_id=%s | parts=%s | wait=%.1fs",
        getattr(channel, "id", "unknown"),
        len(channel_data["pending_parts"]),
...
async def on_message(message):
    ...
    # Склейка подряд идущих сообщений в один запрос к AI.
    if _queue_message_for_ai(
        message.channel,
        channel_data,
        message_text,
        image_urls,
